    ...
    public function onInteract(PlayerInteractEvent $e): void{
        ...
        if($dnbt->hasTag("summon", StringTag::class) and !$e->isCancelled()){
            if(in_array($player->getLevel()->getFolderName(), Configuration::getNotAllowWorlds())){
                $player->sendMessage(TextFormat::RED . "Bạn khoont thể đặt đệ tử ở đây!.");
                return;
            }
            $entity = null;
        ...
        if($this->isLinkable($player)){
            ...
            if(!$block instanceof Chest){
                $player->sendMessage(TextFormat::RED . "Vui lòng nhăp vào chest chứ không phải block: " . $e->getBlock()->getName());
                return;
            }
            ...
            $player->sendMessage(TextFormat::GREEN . "Bạn đã liên kết với chest!");
            $e->setCancelled();
            return;
        }
    }
